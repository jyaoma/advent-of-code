...
input.forEach((line) => {
    ...
    if (operator === '+') {
      if (operand === 'old') {
        currentMonkey.operation = (old) => {
          const res = {};
          Object.keys(old).forEach((den) => {
            res[den] = (old[den] * 2) % den;
          });
          return res;
        };
      } else {
        currentMonkey.operation = (old) => {
          const res = {};
          Object.keys(old).forEach((den) => {
            res[den] = (old[den] + parseInt(operand)) % den;
          });
          return res;
        };
      }
  ...
  if (line === '') {
    ...
    currentMonkey = { counter: 0 };
  }
});
monkeys[currentMonkeyNo] = currentMonkey;

const tests = monkeys.map((monkey) => monkey.test);
console.log(monkeys);
...
for (let round = 0; round < 10000; round++) {
  ...
  monkeys.forEach((monkey, i) => {
    monkey.items.forEach((item) => {
      ...
      const testResult = doOp[monkey.test] === 0;
      if (testResult) {
