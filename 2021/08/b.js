...
// input.forEach((line) => {
input.forEach((line) => {
  const [clues, output] = line.split(" | ");
  // top a, left-top b, right-top c, middle d, left bottom e, right bottom f, bottom g
  const individualClues = clues.split(" ");
  const segmentMap = {};
  const reverseSegmentMap = { " ": " " };
  const numberMap = [];

  // figure out 1, 4, 7, 8
  individualClues.forEach((clue) => {
    if (clue.length === 2) {
      numberMap[1] = clue;
    } else if (clue.length === 3) {
      numberMap[7] = clue;
    } else if (clue.length === 4) {
      numberMap[4] = clue;
    } else if (clue.length === 7) {
      numberMap[8] = clue;
    }
  });

  // figure out top - in 7 but not 1
  const sevenLetters = numberMap[7].split("");
  const oneLetters = numberMap[1].split("");
  const topOptions = sevenLetters.filter(
    (letter) => !oneLetters.includes(letter)
  );
  if (topOptions.length > 1) {
    console.log("broke on top");
    console.log(line);
    return;
  }
  const top = topOptions[0];
  segmentMap.a = top;
  reverseSegmentMap[top] = "a";

  // figure out 3 - has all of 1, one of 4
  const fourLetters = numberMap[4].split("");
  const threeOptions = individualClues
    .map((clue) => {
      let count = 0;

      fourLetters.forEach((letter) => {
        if (
          letter !== oneLetters[0] &&
          letter !== oneLetters[1] &&
          clue.includes(letter)
        ) {
          count++;
        }
      });
    ...
    .filter(
      ({ clue, count }) =>
        clue.length === 5 &&
        clue.includes(oneLetters[0]) &&
        clue.includes(oneLetters[1]) &&
        count === 1
    )
    .map(({ clue }) => clue);

  if (threeOptions.length > 1) {
    console.log("broke on three");
    console.log(line);
    return;
  }
  const three = threeOptions[0];

  numberMap[3] = three;

  // figure out bottom
  const threeLetters = three.split("");
  const bottomOptions = threeLetters.filter(
    (letter) =>
      !oneLetters.includes(letter) &&
      !fourLetters.includes(letter) &&
      !sevenLetters.includes(letter)
  );
  if (bottomOptions.length > 1) {
    console.log("broke on bottom");
    console.log(line);
    return;
  }
  const bottom = bottomOptions[0];
  segmentMap.g = bottom;
  reverseSegmentMap[bottom] = "g";

  // figure out left top
  const leftTopOptions = fourLetters.filter(
    (letter) =>
      !oneLetters.includes(letter) &&
      !threeLetters.includes(letter) &&
      !sevenLetters.includes(letter)
  );

  if (leftTopOptions.length > 1) {
    console.log("broke on bottom");
    console.log(line);
    return;
  }
  const leftTop = leftTopOptions[0];
  segmentMap.b = leftTop;
  reverseSegmentMap[leftTop] = "b";

  // figure out middle
  const middleOptions = threeLetters.filter(
    (letter) =>
      fourLetters.includes(letter) &&
      !oneLetters.includes(letter) &&
      !sevenLetters.includes(letter)
  ...
  const middle = middleOptions[0];
  segmentMap.d = middle;
  reverseSegmentMap[middle] = "d";

  // figure out 2
  const twoOptions = individualClues.filter((clue) => {
    const checkTwoOptions = (letter) =>
      !oneLetters.includes(letter) &&
      !threeLetters.includes(letter) &&
      !fourLetters.includes(letter) &&
      !sevenLetters.includes(letter);
    clueLetters = clue.split("");
    const remainingLetters = clueLetters.filter(checkTwoOptions);
    return remainingLetters.length > 0 && clue.length === 5;
  });
  if (twoOptions.length > 1) {
    console.log("broke on 2");
    console.log(line);
    console.log(twoOptions);
    return;
  }
  const two = twoOptions[0];
  numberMap[2] = two;

  // figure out 5
  const twoLetters = two.split("");
  const fiveOptions = individualClues.filter((clue) => {
    const otherLetters = clue
      .split("")
      .filter(
        (letter) => ![segmentMap.a, segmentMap.d, segmentMap.g].includes(letter)
      );
    return (
      clue.length === 5 &&
      clue.includes(segmentMap.a) &&
      clue.includes(segmentMap.d) &&
      clue.includes(segmentMap.g) &&
      !twoLetters.includes(otherLetters[0]) &&
      !twoLetters.includes(otherLetters[1])
    );
  });
  if (fiveOptions.length > 1) {
    console.log("broke on 5");
    console.log(line);
    console.log(fiveOptions);
    return;
  }
  const five = fiveOptions[0];
  numberMap[5] = five;

  // figure out left bottom
  const fiveLetters = five.split("");
  const leftBottomOptions = twoLetters.filter(
    (letter) =>
      !oneLetters.includes(letter) &&
      !threeLetters.includes(letter) &&
      !fourLetters.includes(letter) &&
      !fiveLetters.includes(letter) &&
      !sevenLetters.includes(letter)
  );
  if (leftBottomOptions.length > 1) {
    console.log("broke on left bottom");
    console.log(line);
    console.log(leftBottomOptions);
    return;
  }
  const leftBottom = leftBottomOptions[0];
  segmentMap.e = leftBottom;
  reverseSegmentMap[leftBottom] = "e";

  // figure out right top
  const rightTopOptions = oneLetters.filter(
    (letter) => !fiveLetters.includes(letter)
  );
  if (rightTopOptions.length > 1) {
    console.log("broke on right top");
    console.log(line);
    console.log(rightTopOptions);
    return;
  }
  const rightTop = rightTopOptions[0];
  segmentMap.c = rightTop;
  reverseSegmentMap[rightTop] = "c";

  // figure out right bottom
  segmentMap.f = "abcdefg"
    .split("")
    .filter(
      (letter) =>
        ![
  ...
  reverseSegmentMap[segmentMap.f] = "f";

  const translated = output
    .split("")
    .map((letter) => reverseSegmentMap[letter])
    .join("");
  const outputDigits = translated.split(" ");
  const segmentToNumMap = {
    abcefg: "0",
    cf: "1",
    acdeg: "2",
    acdfg: "3",
  ...
  const segmentToNum = (segment) => {
    const sorted = segment.split("").sort().join("");
    return segmentToNumMap[sorted];
  };
  const numStr = `${segmentToNum(outputDigits[0])}${segmentToNum(
    outputDigits[1]
  )}${segmentToNum(outputDigits[2])}${segmentToNum(outputDigits[3])}`;
  const outputNum = parseInt(numStr, 10);
  // console.log(numStr);
  total += outputNum;
});
