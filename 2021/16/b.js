...
const rawInputExample1 = "C200B40A82"; // 3
const rawInputExample2 = "04005AC33890"; // 54
const rawInputExample3 = "880086C3E88112"; // 7
const rawInputExample4 = "CE00C43D881120"; // 9
const rawInputExample5 = "D8005AC2A8F0"; // 1
const rawInputExample6 = "F600BC2D8F"; // 0
const rawInputExample7 = "9C005AC2F8F0"; // 0
const rawInputExample8 = "9C0141080250320F1802104A08"; // 1

// const rawInput = rawInputExample7;
...
const recurseCalculatePacket = (packet) => {
  if (packet.type === 4) {
    return packet.literalValue;
  } else {
    packet.packets.forEach((subPacket, index) => {
      if (!subPacket.calculatedValue && !subPacket.literalValue) {
        packet.packets[index].calculatedValue =
          recurseCalculatePacket(subPacket);
      } else if (subPacket.literalValue) {
        packet.packets[index].calculatedValue = subPacket.literalValue;
      }
    });

    if (packet.type === 0) {
      // sum
      return packet.packets.reduce((acc, cur) => acc + cur.calculatedValue, 0);
    } else if (packet.type === 1) {
      // product
      return packet.packets.reduce((acc, cur) => acc * cur.calculatedValue, 1);
    } else if (packet.type === 2) {
      // mininum
      return packet.packets.reduce(
        (acc, cur) => (acc < cur.calculatedValue ? acc : cur.calculatedValue),
      ...
      // greater than
      return packet.packets[0].calculatedValue >
        packet.packets[1].calculatedValue
        ? 1
        : 0;
...
// console.log(packet.calculatedValue);

const value = recurseCalculatePacket(packet);
