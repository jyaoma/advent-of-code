const rawInputPersonal =
  "";

const rawInputExample1 = "D2FE28";
const rawInputExample2 = "38006F45291200";

const rawInput = rawInputExample2;

const hexToBinMap = {
  0: "0000",
  1: "0001",
  2: "0010",
  3: "0011",
  4: "0100",
  5: "0101",
  6: "0110",
  7: "0111",
  8: "1000",
  9: "1001",
  A: "1010",
  B: "1011",
  C: "1100",
  D: "1101",
  E: "1110",
  F: "1111",
};

const input = rawInput
  .split("")
  .map((char) => hexToBinMap[char])
  .join("");

console.log(input);

const packet = {
  data: input,
  packets: [],
};

const bin2Dec = (bin) =>
  bin
    .split("")
    .reduce(
      (acc, cur, idx) =>
        acc + (cur === "1" ? Math.pow(2, bin.length - idx - 1) : 0),
      0
    );

let pointer = 0;

const analyzePackets = (packet) => {
  const { data } = packet;
  const version = bin2Dec(data.slice(pointer, pointer + 3));
  pointer += 3;
  const type = bin2Dec(data.slice(pointer, pointer + 3));
  pointer += 3;

  // literal
  if (type === 4) {
    let firstBit = "1";
    let digits = "";
    do {
      firstBit = data[pointer++];
      digits += data.slice(pointer, pointer + 4);
      pointer += 4;
    } while (firstBit === "1");
    packet.literalValue = bin2Dec(digits);
  } else {
    const indicator = data[pointer++];
    if (indicator === "0") {
      // 15 bits
      const length = bin2Dec(data.slice(pointer, pointer + 15));
      pointer += 15;
    } else if (indicator === "1") {
      // 11 bits
      const length = bin2Dec(data.slice(pointer, pointer + 11));
      pointer += 11;
    }
  }
};

analyzePacket(packet);

console.log(packet);
