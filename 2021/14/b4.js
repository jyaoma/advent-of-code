...
inputPairs.forEach(({ from, to }) => (parsedInputPairs[from] = to));

const result = {};
const increment = Math.pow(2, 40);
inputTemplate.split("").forEach((char, index) => {
  result[increment * index] = char;
});
...
for (let step = 0; step < 40; step++) {
  ...
  for (let i = keys.length - 2; i >= 0; i--) {
    const key0 = keys[i];
    const key1 = keys[i + 1];
    const key = `${result[key0]}${result[key1]}`;
    const newKey = (parseInt(key0, 10) + parseInt(key1, 10)) / 2;

    result[newKey] = parsedInputPairs[key];
  }
  console.timeLog('a')
}
