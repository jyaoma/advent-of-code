...
input.forEach((food) => {
  const rucksack1 = food.substring(0, food.length / 2);
  const rucksack2 = food.substring(food.length / 2);

  let letter = '';
  let tempLetter = null;

  for (let i = 0; i < rucksack1.length; i++) {
    tempLetter = rucksack1.charAt(i);
    if (rucksack2.includes(tempLetter) && !letter.includes(tempLetter))
      letter += tempLetter;
  }

  if (letter.length !== 1) {
    console.log(`${rucksack1} ${rucksack2} ${letter}`);
  }
